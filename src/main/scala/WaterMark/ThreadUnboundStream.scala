...
import WaterMark.UnboundStream1.MyAssignerWithPeriodicWatermarks
...
object ThreadUnboundStream {
  ...
  class MyAssignerWithPeriodicWatermarks extends AssignerWithPeriodicWatermarks[(String, Long)] {
    ...
    override def extractTimestamp(element: (String, Long), previousElementTimestamp: Long): Long = {
      ...
      val result = s"当前记录信息是：$threadId,EventTime→$currentTimestamp，格式化之后的时间→${sdf.format(currentTimestamp)} | 当前窗口中最大的EventTime→$maxTimestamp，格式化之后的时间→${sdf.format(maxTimestamp)} | 当前的watermark→$wmTimestamp，格式化之后的时间→${sdf.format(wmTimestamp)}"
      println(result)
